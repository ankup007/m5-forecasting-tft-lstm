...
```text
p(y_{i,T+1:T+H} | y_{i,T-C+1:T}, x_{i,T-C+1:T+H}, s_i)
```
...
```text
p(y_{i,T+1:T+H} | history, covariates, s_i)
= product_{h=1}^{H} p(y_{i,T+h} | y_{i,T-C+1:T+h-1}, x_{i,T-C+1:T+h}, s_i)
```
...
h_{i,t} = LSTM(h_{i,t-1}, z_{i,t})
theta_{i,t} = W_o h_{i,t} + b_o
y_{i,t} ~ Distribution(theta_{i,t})
```
...
```text
y_{i,t} ~ NegBin(mu_{i,t}, alpha_{i,t})
Var(y_{i,t}) = mu_{i,t} + alpha_{i,t} mu_{i,t}^2
```
...
```text
L_DeepAR = - sum_i sum_t log p(y_{i,t} | theta_{i,t})
```
...
The mathematical weakness is also visible in the factorization. At inference time:

```text
yhat_{i,T+2} depends on yhat_{i,T+1}
yhat_{i,T+3} depends on yhat_{i,T+1:T+2}
...
yhat_{i,T+28} depends on yhat_{i,T+1:T+27}
```
...
```text
yhat_{i,T+h} = f_h(y_{i,T-C+1:T}, x_{i,T-C+1:T+H}, s_i)
for h = 1, ..., H
```
...
```text
yhat_{i,T+h}^{(q)} = q-th conditional quantile of y_{i,T+h}
```
...
L_TFT = sum_i sum_h sum_q L_q(y_{i,T+h}, yhat_{i,T+h}^{(q)})
```
...
```text
v_{j,t} = GRN_j(input_{j,t}, context)
a_t = softmax(GRN_gate([input_{1,t}, ..., input_{m,t}], context))
tilde{x}_t = sum_{j=1}^{m} a_{j,t} v_{j,t}
```
...
```text
GRN(a, c) = LayerNorm(a + Gate(eta))
eta = W_2 ELU(W_1 a + W_c c + b_1) + b_2
Gate(eta) = sigmoid(W_g eta) * W_s eta
```
...
```text
u_t = LSTM_encoder(tilde{x}_{T-C+1:T})
v_h = LSTM_decoder(tilde{x}_{T+1:T+H})
```

Then static enrichment injects static context into the temporal representations:

```text
r_t = GRN([u_t or v_t], c_s)
```
...
```text
DeepAR advantage = p(y_future | y_past, x, s) with a count-friendly likelihood and global sharing.
TFT advantage    = f_h(y_past, x_past, x_future, s) with explicit context-dependent feature use.
```
